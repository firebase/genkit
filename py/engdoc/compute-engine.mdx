...
A step-by-step guide to setting up a Google Cloud Platform VM, authenticating with GitHub, and running Genkit Python samples. This guide walks you through deploying Genkit on a Google Cloud Compute Engine instance to run the context sample.
...
Clone the repository and set up a sample:

```bash
git clone git@github.com:firebase/genkit.git
cd genkit/py/samples/context
```

Install dependencies (requires [uv](https://docs.astral.sh/uv/) and the [genkit CLI](https://firebase.google.com/docs/genkit)):
