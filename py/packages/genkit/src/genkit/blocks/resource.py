...
def define_resource(
    ...
    *,
    ...
    Args:
        ...
        template: URI template for pattern matching.
    ...
    action.matches = create_matcher(uri, template)
...
def dynamic_resource(
    ...
    *,
    ...
    if not resolved_uri:
        raise ValueError('must specify either uri or template')
    ...
    async def wrapped_fn(input_data: ResourceInput, ctx: ActionRunContext) -> ResourcePayload:
        ...
        try:
            ...
            if not template_match:
                raise ValueError(f'input {input_data} did not match template {resolved_uri}')
