...
class Registry:
    ...
    def __init__(self):
        ...
        self._value_by_type_and_name: dict[str, dict[str, Any]] = {}
    ...
    def register_value(self, type: str, name: str, value: Any):
        ...
        Raises:
            ValueError: If a value with the given type and name is already
            registered.
        """
        if type not in self._value_by_type_and_name:
            self._value_by_type_and_name[type] = {}

        if name in self._value_by_type_and_name[type]:
            raise ValueError(
                f'value for type "{type}" '
                f'and name "{name}" is already registered'
            )

        self._value_by_type_and_name[type][name] = value
    ...
    def lookup_value(self, type: str, name: str) -> Any | None:
        ...
        """
        if (
            type in self._value_by_type_and_name
            and name in self._value_by_type_and_name[type]
        ):
            return self._value_by_type_and_name[type][name]
