...
"""Reflection API v1: HTTP-based server for Genkit development tools.

This module provides an HTTP reflection server that exposes endpoints for
health checks, action discovery, and action execution. The CLI and Dev UI
connect to this server as HTTP clients.
...
Key Concepts (ELI5)::
    ...
    ├─────────────────────┼────────────────────────────────────────────────┤
    │ Reflection Server   │ Genkit starts a mini web server that lets     │
    │                     │ tools peek inside and run your AI flows.       │
    ├─────────────────────┼────────────────────────────────────────────────┤
    │ Action              │ Any registered flow, tool, or model that can  │
    │                     │ be discovered and executed via this API.       │
    ├─────────────────────┼────────────────────────────────────────────────┤
    │ Streaming Response  │ Results sent piece by piece, like watching    │
    │                     │ a video stream instead of downloading first.   │
    └─────────────────────┴────────────────────────────────────────────────┘
...
Data Flow::
    ...
    CLI / Dev UI                         Reflection Server
         ...
         │  ◄──── {actions dict} ────         │
         │                                    │
         │  ──── POST /api/runAction ────►    │  (key, input, context)
         │  ◄──── X-Genkit-Trace-Id header ── │  (early flush)
         │  ◄──── stream chunks ────          │  (if streaming)
         │  ◄──── final result ────           │
         │                                    │
         │  ──── POST /api/cancelAction ──►   │  (traceId)
         │  ◄──── {"message": "..."} ────     │
         │                                    │
...
Key Endpoints::
    ...
    │ GET    │ /api/values         │ List values by type                   │
    │ GET    │ /api/envs           │ List configured environments          │
    │ POST   │ /api/runAction      │ Execute action (supports streaming)   │
    │ POST   │ /api/cancelAction   │ Cancel a running action by traceId    │
    │ POST   │ /api/notify         │ Handle notifications                  │
    │ POST   │ /api/__quitquitquit │ Trigger graceful shutdown             │
    └────────┴─────────────────────┴───────────────────────────────────────┘
...
See Also:
    - V2 WebSocket implementation: genkit.core.reflection
"""
