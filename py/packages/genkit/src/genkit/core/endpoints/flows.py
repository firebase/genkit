...
from genkit.web.sse import sse_wrapper
...
def create_flows_asgi_app(
    ...
    async def run_flow(request: Request) -> JSONResponse | StreamingResponse:
        ...
        Returns:
            A JSON or StreamingResponse with the flow result, or an error
            response.
        """
    ...
    async def handle_streaming_flow(
        ...
        Returns:
            A StreamingResponse with the flow result or error.
        """

        async def event_stream_generator() -> AsyncGenerator[dict, None]:
        ...
        return StreamingResponse(
            sse_wrapper(event_stream_generator()),
    ...
    routes = [
        ...
        Route('/list', list_flows, methods=['GET']),  # TODO: Check requirement.
        Route('/{flow_name:path}', run_flow, methods=['POST']),
    ]
