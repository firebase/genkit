...
_loop_clients: MutableMapping[asyncio.AbstractEventLoop, dict[str, httpx.AsyncClient]] = (
...
async def close_cached_clients(cache_key: str | None = None) -> None:
    ...
    if cache_key is not None:
        # Close specific client
        if cache_key in loop_cache:
            client = loop_cache.pop(cache_key)
            await client.aclose()
            logger.debug('Closed cached client', cache_key=cache_key)
    else:
        # Close all clients in this loop's cache
        for key, client in list(loop_cache.items()):
            await client.aclose()
