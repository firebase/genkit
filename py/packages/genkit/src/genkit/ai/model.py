...
# TypeVar for generic output type in ModelResponse
...
class ModelResponseChunk(GenerateResponseChunk):
    ...
    chunk_parser: Callable[[ModelResponseChunk], object] | None = Field(None, exclude=True)
    ...
    def __init__(
        ...
        index: int,
        chunk_parser: Callable[[ModelResponseChunk], object] | None = None,
    ) -> None:
        """Initializes the ModelResponseChunk.
...
GenerateResponseWrapper = ModelResponse
GenerateResponseChunkWrapper = ModelResponseChunk
...
def define_model(
    ...
    Args:
        registry: The registry to register the model in.
...
# Backward compat alias (deprecated, use ModelConfig)
GenerationCommonConfig = ModelConfig
