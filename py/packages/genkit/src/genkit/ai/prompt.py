...
class PromptGenerateOptions(TypedDict, total=False):
    ...
    Attributes:
        ...
        on_chunk: Callback function called with each response chunk during
            streaming. Signature: `(chunk: ModelResponseChunk) -> None`
...
class ModelStreamResponse(Generic[OutputT]):
    ...
    ModelStreamResponse, with both stream and response properties
    accessible simultaneously.

    When the prompt has a typed output schema, `response` returns
    `ModelResponse[OutputT]` with typed `.output` property.

    Overview:
        When you call `prompt.stream()`, you get a ModelStreamResponse
        that allows you to:
    ...
    Attributes:
        ...
        response: Awaitable that resolves to the complete ModelResponse
    ...
    See Also:
        - JavaScript ModelStreamResponse: js/ai/src/generate.ts
    """
    ...
    def __init__(
        ...
        Args:
            channel: The channel providing response chunks. This is an async
                iterable that yields ModelResponseChunk objects.
...
class ExecutablePrompt(Generic[InputT, OutputT]):
    ...
    def __init__(
        ...
        self._config: dict[str, Any] | ModelConfig | None = config
    ...
    def stream(
        ...
        *,
        ...
        returning a ModelStreamResponse with both stream and response properties.
...
def define_prompt(  # pyright: ignore[reportInconsistentOverload]
    ...
    Args:
        ...
        config: The generation configuration.
        description: A description of the prompt.
        input_schema: The input schema for the prompt (Pydantic model, JSON schema dict, or registered schema name).
        ...
        output_instructions: Instructions for formatting the output.
        output_schema: The output schema (Pydantic model, JSON schema dict, or registered schema name).
        output_constrained: Whether the output should be constrained to the output schema.
        max_turns: The maximum number of turns in a conversation.
        return_tool_requests: Whether to return tool requests.
        metadata: Metadata to associate with the prompt.
    ...
    Returns:
        An ExecutablePrompt instance. When `input_schema=type[I]` and
        `output_schema=type[O]` are provided, returns `ExecutablePrompt[I, O]`
...
def define_schema(registry: Registry, name: str, schema: type[BaseModel]) -> None:
    ...
    Example:
        ```python
        from genkit.ai.prompt import define_schema
...
def load_prompt(registry: Registry, path: Path, filename: str, prefix: str = '', ns: str = '') -> None:
    ...
    async def load_prompt_metadata() -> dict[str, Any]:
        ...
        return {
            ...
            'input': {
                'default': input_schema.get('default') if isinstance(input_schema, dict) else None,
                'jsonSchema': input_schema.get('schema') if isinstance(input_schema, dict) else None,
            },
            'metadata': metadata,
