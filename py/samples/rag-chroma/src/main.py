...
"""RAG demo using ChromaDB with Genkit - Cat Knowledge Base.

This sample demonstrates Retrieval-Augmented Generation (RAG) using ChromaDB
as an in-memory vector store. It includes a cat-themed knowledge base with
...
from genkit.plugins.chroma import chroma, chroma_indexer_ref, chroma_retriever_ref
...
async def index_cat_knowledge_flow(input: IndexInput) -> IndexResult:
    """Index cat knowledge documents into ChromaDB.

    This flow loads cat-themed documents from the ./data/ directory and
    indexes them into ChromaDB for retrieval. Documents are split into
    ...
    """
    await logger.ainfo('Starting index_cat_knowledge_flow', input=input.model_dump())
    all_texts: list[str] = []
    sources: list[str] = []
    ...
    try:
        ...
        if input.include_care_guide:
            await logger.ainfo('Loading cat care guide')
            ...
            if care_chunks:
                ...
                await logger.ainfo('Loaded cat care guide', chunk_count=len(care_chunks))

        # Load famous cats
        if input.include_famous_cats:
            await logger.ainfo('Loading famous cats')
            ...
            if famous_chunks:
                ...
                await logger.ainfo('Loaded famous cats', chunk_count=len(famous_chunks))

        # Load cat facts
        if input.include_cat_facts:
            await logger.ainfo('Loading cat facts')
            facts = load_json_facts(DATA_DIR / 'cat_facts.json')
            all_texts.extend(facts)
            if facts:
                sources.append(f'cat_facts.json ({len(facts)} facts)')
                await logger.ainfo('Loaded cat facts', fact_count=len(facts))

        if not all_texts:
            await logger.awarning('No documents found to index')
            return IndexResult(
                total_documents=0,
                sources_indexed=[],
        ...
        await logger.ainfo('Converted texts to documents', document_count=len(documents))

        # Index into ChromaDB
        await logger.ainfo('Indexing documents into ChromaDB', collection=COLLECTION_NAME)
        await ai.index(
            indexer=chroma_indexer_ref(collection_name=COLLECTION_NAME),
            documents=documents,
        )
        await logger.ainfo('Successfully indexed documents', document_count=len(documents))

        return IndexResult(
            total_documents=len(documents),
            sources_indexed=sources,
            message=f'Successfully indexed {len(documents)} documents into "{COLLECTION_NAME}" collection',
        )
    except Exception as e:
        await logger.aexception('Error during indexing', error=str(e))
        raise
...
async def ask_about_cats_flow(input: QueryInput) -> QueryResult:
    ...
    result = await ai.retrieve(
        retriever=chroma_retriever_ref(collection_name=COLLECTION_NAME),
        query=input.question,
        options={'k': input.num_results},
    )
