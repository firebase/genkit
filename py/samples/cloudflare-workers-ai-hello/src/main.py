...
# Initialize Genkit with CF Workers AI plugin
ai = Genkit(
...
async def describe_image(input: ImageDescribeInput) -> str:
    ...
    response = await ai.generate(
        ...
        model=cloudflare_model('@cf/meta/llama-4-scout-17b-16e-instruct'),
        messages=[
            Message(
                role=Role.USER,
                content=[
...
async def say_hi_with_config(input: ConfigDemoInput) -> str:
    ...
    This flow demonstrates using CloudflareConfig for fine-tuned control over
