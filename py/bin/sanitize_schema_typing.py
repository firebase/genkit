...
class ClassTransformer(ast.NodeTransformer):
    ...
    def create_model_config(
        self, existing_config: ast.Call | None = None
    ) -> ast.Assign:
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> ast.ClassDef:  # noqa: N802
        ...
        if existing_model_config_assign and isinstance(
            existing_model_config_assign.value, ast.Call
        ):
        ...
        if self.is_rootmodel_class(node):
            ...
            if not added_config:
                ...
                insert_pos = (
                    1
                    if len(new_body) > 0 and isinstance(new_body[0], ast.Expr)
                    else 0
                )
