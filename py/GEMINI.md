...
## Plugin API Conformance Verification

This section documents the verification status of each Python plugin against official provider
documentation. Last verified: 2026-02-03.
...
| mistral | ⚠️ Gaps | Agents API, Codestral FIM | None | Good | Medium |
...
- ✅ API key not logged
- ✅ Credentials handled via ADC or explicit params
- ⚠️ Consider adding request/response sanitization for PII
...
| Gap | Description | Impact | Priority |
|-----|-------------|--------|----------|
| Cache control (ephemeral) | `cache_control` with TTL not supported | High - cost savings | High |
| PDF/Document support | `DocumentBlockParam` not implemented | High - common use case | High |
| Citations | Citation extraction not supported | Medium | Medium |
| Web search tool | Server-side `web_search` tool not supported | Medium | Medium |
| Batch API | Message batches not supported | Low - async processing | Low |
| URL image source | Only base64 images, not URL references | Medium | Medium |

**Code Fix Required** (cache control):
```python
# Current: No cache_control support
# Should add to _to_anthropic_messages():
if hasattr(part, 'cache_control') and part.cache_control:
    block['cache_control'] = {'type': 'ephemeral', 'ttl': part.cache_control.ttl}
```

**Security Considerations**:
- ✅ API key from env or param
- ⚠️ No input sanitization for prompt injection
...
| Gap | Description | Impact | Priority |
|-----|-------------|--------|----------|
| Guardrails | Bedrock Guardrails not integrated | Medium - content filtering | Medium |
| Knowledge bases | RAG via Bedrock KB not supported | Medium | Medium |
| Model invocation logging | CloudWatch logging config | Low | Low |

**Security Considerations**:
- ✅ Uses boto3 credential chain (IAM, profiles, env vars)
- ✅ Supports inference profiles for API key auth
- ⚠️ Long read timeout (3600s) for Nova - consider making configurable
...
| Gap | Description | Impact | Priority |
|-----|-------------|--------|----------|
| Vision in chat | Images via chat API need testing | Low - works via generate | Low |
| Pull models | Model download/management | Low - user manages | Low |
| Custom model files | Modelfile creation | Low | Low |

**Security Considerations**:
- ⚠️ Default server is localhost - ensure not exposed publicly
- ✅ Supports custom server address
- ✅ Client factory pattern prevents event loop issues
...
| Gap | Description | Impact | Priority |
|-----|-------------|--------|----------|
| Agents API | Mistral Agents endpoint not supported | High - agentic workflows | High |
| FIM (Fill-in-Middle) | Codestral FIM for code completion | Medium - code use cases | Medium |
| Built-in tools | websearch, code_interpreter, image_generation | Medium | Medium |
| Embeddings | mistral-embed model | Medium - common use case | Medium |
| MCP orchestration | Model Context Protocol | Low | Low |

**Code Fix Needed** (embeddings):
```python
# Add to plugin.py:
...
**Security Considerations**:
- ✅ API key from env or param
- ⚠️ `safe_prompt` / `safe_mode` params exist but may not be properly validated
...
| Gap | Description | Impact | Priority |
|-----|-------------|--------|----------|
| Agent Tools API | Server-side and client-side tool calling (Jan 2026) | High - new feature | High |
| Web search options | Built-in web search configuration | Medium | Medium |
| Deferred execution | `deferred` parameter handling | Low | Low |
| New models | grok-4-1-fast-reasoning, grok-4-1-fast-non-reasoning | Medium | Medium |

**Security Considerations**:
- ✅ API key from env or param
- ⚠️ Uses xai_sdk protobuf - ensure version compatibility
...
| Gap | Description | Impact | Priority |
|-----|-------------|--------|----------|
| reasoning_content | CoT output not extracted/exposed | **Critical** - core R1 feature | **Critical** |
| Parameter validation | R1 ignores temp/top_p but no warning | High - silent failures | High |
| Multi-round reasoning | Must strip reasoning_content from context | High - breaks multi-turn | High |
| Tool calling in R1 | Not supported in reasoner mode | Medium - documented limitation | Medium |
| Strict mode | Beta strict JSON schema mode | Low | Low |
...
# Should add custom response handling:
class DeepSeekModel:
...
**Security Considerations**:
- ⚠️ **Parameter Silently Ignored**: Setting `temperature`, `top_p` on R1 has no effect
  but doesn't error - could lead to unexpected behavior
- ✅ API key from env or param
...
| Gap | Description | Impact | Priority |
|-----|-------------|--------|----------|
| Async Batch API | Batch processing endpoint | Low - async jobs | Low |
| Fine-tuning | Model fine-tuning API | Low | Low |
| AI Gateway integration | Caching/rate limiting | Low - infra concern | Low |

**Security Considerations**:
- ✅ Account ID and API token required
- ✅ Bearer token auth
- ⚠️ Media URLs fetched server-side - potential SSRF if not validated
...
| Gap | Description | Impact | Priority |
|-----|-------------|--------|----------|
| Inference Endpoints | Dedicated endpoints not supported | High - production use | High |
| Text Generation Inference | TGI-specific features | Medium | Medium |
| Streaming | May need verification | Medium | Medium |
...
| Gap | Description | Impact | Priority |
|-----|-------------|--------|----------|
| Azure AI Studio | New unified API | High - Microsoft direction | High |
| Content filtering | Azure content safety | Medium | Medium |
...
1. **Critical**: Fix DeepSeek R1 `reasoning_content` extraction
2. **High**: Add Anthropic cache control and document support
3. **High**: Add Mistral embeddings support
4. **Medium**: Add xAI Agent Tools API support
5. **Medium**: Add parameter validation warnings for DeepSeek R1
6. **Medium**: Improve test coverage across all plugins
7. **Low**: Add SSRF protection for media URL fetching
