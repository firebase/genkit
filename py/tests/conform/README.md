...
py/tests/conformance/test-model-conformance anthropic

# Test all plugins (requires ALL env vars below)
py/tests/conformance/test-model-conformance --all
```
...
`py/bin/check_consistency` (Check 21) verifies that every model plugin
(any plugin with a `model_info.py`) has a conformance spec and entry point.
This check runs as part of `bin/lint` and does **not** require API keys — it
only checks that the files exist.

The actual conformance tests (`py/tests/conformance/test-model-conformance`) require live
API keys and are run manually, not as part of CI lint.

## Adding a new plugin

1. Create `tests/conformance/<plugin>/model-conformance.yaml` with the models
   and capabilities to test.
2. Create `tests/conformance/<plugin>/conformance_entry.py` that initializes
   Genkit with the plugin and keeps the process alive.
3. Add the plugin's required env vars to `py/tests/conformance/test-model-conformance`.
4. Run `py/bin/check_consistency` to verify the files are detected.
...
```
tests/conformance/google-genai/model-conformance.yaml
