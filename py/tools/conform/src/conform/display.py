...
# ---------------------------------------------------------------------------
# Progress table (live-updated during ``conform run``)
# ---------------------------------------------------------------------------


def build_progress_table(results: dict[str, object]) -> Table:
    """Build a Rich table showing current status of all plugins.

    *results* is a dict mapping plugin names to ``PluginResult`` dataclass
    instances (import-free signature to avoid circular imports).
    """
    from conform.runner import PluginResult
    ...
    for _plugin, raw_result in results.items():
        result: PluginResult = raw_result  # type: ignore[assignment]
        emoji = status_emoji(result.status)
        style = status_style(result.status)

        status_text = Text(result.status.value.upper() if result.status != Status.RUNNING else 'running…', style=style)
        ...
        table.add_row(emoji, result.plugin, status_text, elapsed_str(result.elapsed_s), Text(detail, style='dim'))

    return table


# ---------------------------------------------------------------------------
# Summary table (printed once after ``conform run`` completes)
# ---------------------------------------------------------------------------


def build_summary_table(results: dict[str, object]) -> Table:
    ...
    for _plugin, raw_result in results.items():
        ...
        table.add_row(emoji, result.plugin, Text(label, style=style), elapsed_str(result.elapsed_s), detail)

    return table


def print_summary_footer(results: dict[str, object]) -> None:
    """Print the aggregated pass/fail/skip/error counts with emojis."""
    from conform.runner import PluginResult

    passed = sum(1 for r in results.values() if isinstance(r, PluginResult) and r.status == Status.PASSED)
    failed = sum(1 for r in results.values() if isinstance(r, PluginResult) and r.status == Status.FAILED)
    errors = sum(1 for r in results.values() if isinstance(r, PluginResult) and r.status == Status.ERROR)
    skipped = sum(1 for r in results.values() if isinstance(r, PluginResult) and r.status == Status.SKIPPED)
    total_time = max(
        (r.elapsed_s for r in results.values() if isinstance(r, PluginResult)),
        default=0.0,
    )
