...
Provides three subcommands:
    ...
    conform check-model [PLUGIN...] [--all] [-j N] [-v]
...
from conform.display import Status, console, rust_error, stdout_console
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog='conform',
        description=(
            'Parallel model conformance test runner for Genkit Python plugins.\n\n'
...
def _cmd_check_model(args: argparse.Namespace, config: ConformConfig) -> int:
    ...
    console.print(
        ...
        f'concurrency={config.concurrency}[/dim]'
    )
    console.print()
...
def _cmd_list(config: ConformConfig) -> int:
    ...
    # Print to stdout so it appears in the right order when
    # appended after argparse help output.
...
def main() -> None:
    ...
    config = load_config(concurrency_override=concurrency_override)
