...
def _resolve_runtime_names(runtime_args: list[str] | None) -> list[str]:
    """Resolve which runtimes to use.

    Args:
        runtime_args: Explicit runtime name(s), or ``None`` for all
            configured.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    global_parser.add_argument(
        ...
        metavar='NAME',
        help=(
            'Runtime(s) to use (e.g. python, js, go). '
            'Multiple values accepted. If not specified, runs all configured runtimes.'
        ),
    )
    global_parser.add_argument(
        '--specs-dir',
...
def main() -> None:
    ...
    runtime_args: list[str] | None = getattr(args, 'runtime', None)

    if args.command is None:
        parser.print_help()
        _cmd_list(_resolve_runtime_names(runtime_args))
        sys.exit(0)

    runtime_names = _resolve_runtime_names(runtime_args)
