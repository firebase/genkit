...
import pytest
from releasekit.backends.validation import Validator
...
def _manifest(*names_versions: tuple[str, str]) -> ReleaseManifest:
    ...
    return ReleaseManifest(
        git_sha='abc123',
        packages=[
            PackageVersion(name=n, old_version='0.0.0', new_version=v, bump='minor')
...
class TestRunValidatorsIntegration:
    ...
    def test_both_pass_on_cyclonedx(self) -> None:
        from releasekit.backends.validation import run_validators
    ...
    def test_both_formats_validated(self) -> None:
        from releasekit.backends.validation import all_passed, run_validators
