...
class SLSALevelValidator:
    ...
    def _validate_context(self, ctx: Any) -> ValidationResult:  # noqa: ANN401
        ...
        if self.target_level >= 2:
            if not ctx.is_ci:
                issues.append(
                    'L2 requires a hosted build platform (currently local build)'
                )
            if not ctx.builder_id:
                issues.append('L2 requires builder_id to be set')
            if not ctx.source_repo:
                issues.append(
                    'L2 requires source_repo for provenance authenticity'
                )
            if not ctx.source_digest:
                issues.append(
                    'L2 requires source_digest (git commit SHA)'
                )
        ...
        if self.target_level >= 3:
            if not ctx.runner_environment:
                issues.append(
                    'L3 requires runner_environment to be known'
                )
            elif ctx.ci_platform == 'github-actions':
                if ctx.runner_environment != 'github-hosted':
                    issues.append(
                        'L3 requires github-hosted runners '
                        f'(got {ctx.runner_environment!r})'
                    )
            if not ctx.source_entry_point:
                issues.append(
                    'L3 requires source_entry_point (build-as-code: '
                    'build config must be in version control)'
                )
            if not ctx.invocation_id:
                issues.append(
                    'L3 requires invocation_id for build traceability'
                )
            if not ctx.source_ref:
                issues.append(
                    'L3 requires source_ref for complete '
                    'externalParameters'
                )
        ...
        if issues:
            return ValidationResult.failed(
                self.name,
                f'SLSA Build L{self.target_level} not achieved '
                f'(reached L{achieved}): {len(issues)} issue(s)',
    ...
    def _validate_provenance(self, stmt: dict[str, Any]) -> ValidationResult:
        ...
        if self.target_level >= 2:
            ...
            if bid and not (bid.startswith('https://') or bid.startswith('local://')):
                issues.append(
                    f'L2: builder.id should be a URI (got {bid!r})'
                )
            if not builder.get('version'):
                issues.append(
                    'L2: builder.version should be populated '
        ...
        if self.target_level >= 3:
            ...
            if not ext.get('repository'):
                issues.append(
                    'L3: externalParameters.repository is required '
                    '(complete external parameters)'
                )
            if not ext.get('ref'):
                issues.append(
                    'L3: externalParameters.ref is required '
                    '(complete external parameters)'
                )
            ...
            has_source_dep = any(
                d.get('digest', {}).get('gitCommit') for d in deps
            )
            if not has_source_dep:
                issues.append(
                    'L3: resolvedDependencies should include source '
                    'with gitCommit digest'
                )
            ...
            if not version.get('runnerEnvironment'):
                issues.append(
                    'L3: builder.version.runnerEnvironment is required '
            ...
            if recorded_level is None:
                issues.append(
                    'L3: internalParameters.slsaBuildLevel should be '
                    'recorded for compliance tracking'
                )
            elif recorded_level < 3:
                issues.append(
                    f'L3: recorded slsaBuildLevel is {recorded_level}, '
                    f'expected >= 3'
                )
            ...
            if not metadata.get('invocationId'):
                issues.append(
                    'L3: metadata.invocationId is required '
