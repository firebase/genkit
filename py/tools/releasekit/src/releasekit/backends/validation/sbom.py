...
class CycloneDXSchemaValidator:
    ...
    def _structural_check(data: dict[str, Any]) -> list[str]:
        ...
        if data.get('bomFormat') != 'CycloneDX':
            issues.append(
                f'bomFormat must be "CycloneDX", got {data.get("bomFormat")!r}'
            )
        ...
        if components is not None and not isinstance(components, list):
            issues.append(
                f'components must be an array, got {type(components).__name__}'
            )
...
class SPDXSchemaValidator:
    ...
    def _structural_check(data: dict[str, Any]) -> list[str]:
        ...
        if packages is not None and not isinstance(packages, list):
            issues.append(
                f'packages must be an array, got {type(packages).__name__}'
            )
        if isinstance(packages, list):
            for i, pkg in enumerate(packages):
                ...
                if 'downloadLocation' not in pkg:
                    issues.append(
                        f'packages[{i}] missing required field "downloadLocation"'
                    )
        return issues
