...
def _write_gradle_properties_version(props_path: Path, new_version: str) -> str:
    ...
    pattern = re.compile(r'^(VERSION_NAME\s*=\s*)(.+?)(\s*)$', re.MULTILINE)
...
def _read_version_catalog_version(
    ...
    # Simple TOML parsing: find [versions] section and the key within it.
    in_versions = False
    for line in text.splitlines():
        ...
        if stripped.startswith('[') and in_versions:
            break
        if in_versions and stripped.startswith(version_key):
            m = re.match(rf'^{re.escape(version_key)}\s*=\s*["\']([^"\']+)["\']', stripped)
            if m:
                return m.group(1)
    return ''
