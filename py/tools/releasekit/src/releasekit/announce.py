...
Usage::
    ...
    await send_announcements(
        cfg, version='1.2.0', packages=['genkit'], event='rollback',
    )
"""
...
_DEFAULT_TEMPLATE = '🚀 Released ${version}: ${packages}'
_DEFAULT_ROLLBACK_TEMPLATE = '⚠️ Rolled back ${version}: ${packages}'


@dataclass(frozen=True)
class AnnouncementConfig:
    """Configuration for release announcements.
    ...
    Attributes:
        ...
        irc_webhook: IRC webhook/bridge URL (HTTP bridge that accepts
        ...
        overrides: Per-group or per-package overrides. Keys are group
            names or exact package names. Non-empty fields in the
            override replace the corresponding base config field.
    """
    ...
    rollback_template: str = _DEFAULT_ROLLBACK_TEMPLATE
    enabled: bool = True
...
async def send_announcements(
    ...
    *,
    ...
    template = (
        config.rollback_template
        if event == 'rollback' and config.rollback_template
        else config.template
    )
