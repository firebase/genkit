...
test.describe('Prompt Queue', () => {
    ...
    async function injectQueueItem(page: Page, content: string) {
        await page.evaluate((msg) => {
            ...
            if (!el) return;
            const comp = (window as unknown as { ng: { getComponent: (el: Element) => unknown } }).ng.getComponent(el) as {
                chatService: {
    ...
    test('queue should be scrollable with many items', async ({ page }) => {
        ...
        const maxHeight = await queueItems.evaluate(el =>
...
test.describe('Message Display', () => {
    ...
    test('should have proper spacing between messages and input', async ({ page }) => {
        ...
        const paddingBottom = await messagesContainer.evaluate(el =>
