...
export class ChatInputComponent implements AfterViewInit {
    ...
    private processFiles(files: File[]): void {
        ...
        if (currentFiles.length + files.length > CHAT_CONFIG.maxAttachments) {
            this.snackBar.open(
                this.translate.instant('errors.tooManyFiles', { max: CHAT_CONFIG.maxAttachments }),
                'OK',
                { duration: 3000 }
            );
            return;
        ...
        for (const file of files) {
            // Check file size
            if (file.size > CHAT_CONFIG.maxFileSizeBytes) {
                this.snackBar.open(
