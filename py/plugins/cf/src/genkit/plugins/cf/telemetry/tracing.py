...
API Token Resolution Order:
    ...
    2. CF_API_TOKEN environment variable (same as used by cf-ai plugin)
...
Usage:
    ...
    add_cf_telemetry(endpoint='https://otel.example.com/v1/traces')

    # Enable input/output logging (disable PII redaction)
    add_cf_telemetry(log_input_and_output=True)

    # Force export in dev environment
    add_cf_telemetry(force_dev_export=True)
    ```
    ...
    def __init__(
        ...
        if self.endpoint is None:
            raise ValueError(
                ...
                'or pass endpoint parameter to add_cf_telemetry().'
            )
