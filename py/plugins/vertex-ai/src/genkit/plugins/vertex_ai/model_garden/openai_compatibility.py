    ...
    def to_openai_messages(self, messages: list[Message]) -> list[ChatMessage]:
        ...
        return [
            ChatMessage(
                ...
                content=''.join(part.root.text for part in m.content if part.root.text is not None),
            )
            for m in messages
        ]

    def generate(self, request: GenerateRequest, ctx: ActionRunContext) -> GenerateResponse:
        openai_config: dict = {'messages': self.to_openai_messages(request.messages), 'model': self._model}
        if ctx.is_streaming:
