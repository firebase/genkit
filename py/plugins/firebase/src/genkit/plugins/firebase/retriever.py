...
from genkit.plugins.firebase.constant import (
    DistanceMeasure,
...
class FirestoreRetriever:
    ...
    def _to_content(
        self, doc_snapshot: DocumentSnapshot
    ) -> list[Dict[str, str]]:
    ...
    def _to_document(self, doc_snapshot: DocumentSnapshot) -> Document:
        ...
        metadata.update({
            ...
            if k != vector_field and k != content_field_name
        })
    ...
    async def retrieve(self, request: RetrieverRequest, _: ActionRunContext):
        ...
        query_embedding_result = await self.embedding_model.embed([
            Document(content=[TextPart(text=request.query)])
        ])
        if not query_embedding_result.embeddings:
            return {'documents': []}
        query_embedding = query_embedding_result.embeddings[0].embedding

        collection_ref = self.firestore_client.collection(
            self.params.get('collection')
        )
        vector_field = self.params.get('vector_field')
        distance_measure_str = self.params.get(
            'distance_measure', 'COSINE'
        ).upper()
        limit = (
            request.limit
            if request.limit is not None
            else (request.top_k if request.top_k is not None else 10)
        )
        ...
        try:
            ...
            print(
                f'Invalid distance measure: {distance_measure_str}. Using COSINE.'
            )
