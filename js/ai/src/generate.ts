...
export class GenerateResponse<O = unknown> implements GenerateResponseData {
  ...
  output(index?: number): O | null {
    const format = this.request?.output?.format || 'json';
    const parser = getFormatParser(format) as FormatParser<O>;

    if (!parser) {
      throw new Error(`Unsupported format: ${format}`);
    }

    return parser.parseResponse(this, index);
  }
  ...
  implements GenerateResponseChunkData
{
  ...
  output(): T | null {
    const format = this.request?.output?.format || 'json';
    const parser = getFormatParser(format) as FormatParser<T>;
...
>(
  ...
  if (parser && parser.instructions) {
    const formatInstructions = parser.instructions(resolvedOptions);
    ...
    resolvedOptions.streamingCallback
      ? (chunk: GenerateResponseChunkData) => {
          ...
          if (resolvedOptions.streamingCallback) {
            resolvedOptions.streamingCallback!(
              new GenerateResponseChunk(chunk, accumulatedChunks, request)
            );
          }
        }
      : undefined,
