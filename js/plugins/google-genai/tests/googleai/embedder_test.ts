...
describe('defineGoogleAIEmbedder', () => {
  ...
  it('defines an embedder with the correct name and info for known model', () => {
    const embedder = defineEmbedder('text-embedding-004', defaultPluginOptions);
    assert.strictEqual(embedder.__action.name, 'googleai/text-embedding-004');
    assert.strictEqual(embedder.__action.metadata?.info?.dimensions, 768);
  });
  ...
  it('defines an embedder with a custom name', () => {
    ...
    assert.strictEqual(
      ...
      'googleai/custom-embedding-model'
    );
  });
  ...
  it('handles custom name with prefix', () => {
    ...
    assert.strictEqual(embedder.__action.name, 'googleai/custom-model');
  });
  ...
  describe('API Key Handling', () => {
    ...
    it('uses API key from call options if apiKey is false in pluginOptions', async () => {
      ...
      await embedder({
        input: [new Document({ content: [{ text: 'test' }] })],
        options: { apiKey: 'call-time-api-key' },
      });
      sinon.assert.calledOnce(fetchStub);
      const fetchOptions = fetchStub.lastCall.args[1];
      assert.strictEqual(
  ...
  describe('Embedder Functionality', () => {
    ...
    it('calls embedContent with taskType, title, and outputDimensionality options', async () => {
      ...
      await embedder({ input: [testDoc1], options: config });
    ...
    it('uses the correct model name in the URL', async () => {
      ...
      await embedder({ input: [testDoc1], options: {} });
