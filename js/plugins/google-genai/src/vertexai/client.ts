...
export async function generateContent(
  ...
  // //return response.json() as Promise<GenerateContentResult>;
...
async function makeRequest(
  ...
  try {
    console.log('DEBUGGG: making request to : ' + url);
    const response = await fetch(url, fetchOptions);
    if (!response.ok) {
      console.log(
        'DEBUGGG: makeRequest ERROR response: ' +
          JSON.stringify(response, null, 2)
      );
      console.dir(response); // DEBUGGG
    ...
    console.log(
      'DEBUGGG: makeRequest GOOD response: ' + JSON.stringify(response, null, 2)
    );
    console.dir(response); // DEBUGGG
...
async function* generateResponseSequence(
  ...
  while (true) {
    ...
    yield enhanceContentResponse(addMissingIndexAndRole(value));
  }
}
