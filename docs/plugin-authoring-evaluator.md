...
Firebase Genkit can be extended to support custom use cases that leverage an LLM as a judge or a purely programatic evaluation of test case output.

## Evaluator definition

Evaluators are functions that assess the content given to and generated by an LLM. There are two main approaches to automated evaluation (testing) - hueristic assesment and LLM-based assesment. In a hueristic approach, define a deterministic function much like those of traditional software development. In an LLM-based assesment, the content is fed back to an LLM and the LLM is asked to score the output according to criteria set in a prompt.

### LLM based Evaluators

An LLM-based evaluator leverages an LLM to evaluate the input, context, or output of your genAI feature.
...
>(
  ...
  try {
    ...
    console.debug(
      `Deliciousness evaluation failed with error ${err} for sample ${JSON.stringify(
        d
      )}`
    );
    throw err;
...
### Non-LLM-based Evaluators

An non-LLM-based evaluator is any function used evaluate the input, context, or output of your genAI feature.

Non-LLM-based evaluators in Genkit are made up of 2 components:
...
export async function usPhoneRegexScore(
  ...
  try {
    ...
    console.debug(
      ` regex matcher failed with error ${err} for sample ${JSON.stringify(d)}`
    );
    throw err;
  }
}
...
To get a sense of whether the custom evaluator performing at the expected level, create a set of test cases that have a clear right and wrong answer.
