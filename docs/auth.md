...
or Firebase provides auth context into the flow where it can do its own
checks. For non-Functions flows, auth can be managed and set as well through
middleware.
...
The Cloud Functions for Firebase SDK Firebase supports Genkit including
...
When using the SDK, the Firebase Auth header is automatically included as
...
When using the `onCallGenkit`, `context.auth` is returned as an object with
...
1.  Use `startFlowsServer()` available using `@genkit-ai/express` plugin and
    provide Express auth middleware in the flow server config:
