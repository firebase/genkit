...
func DefineIndexer(reg *registry.Registry, provider, name string, index func(context.Context, *IndexerRequest) error) Indexer {
	f := func(ctx context.Context, req *IndexerRequest) (struct{}, error) {
		return struct{}{}, index(ctx, req)
	}
	return (*indexerActionDef)(core.DefineAction(reg, provider, name, atype.Indexer, nil, f))
}

// IsDefinedIndexer reports whether an [Indexer] is defined.
func IsDefinedIndexer(reg *registry.Registry, provider, name string) bool {
	return (*indexerActionDef)(core.LookupActionFor[*IndexerRequest, struct{}, struct{}](reg, atype.Indexer, provider, name)) != nil
}

// LookupIndexer looks up an [Indexer] registered by [DefineIndexer].
// It returns nil if the model was not defined.
func LookupIndexer(reg *registry.Registry, provider, name string) Indexer {
	return (*indexerActionDef)(core.LookupActionFor[*IndexerRequest, struct{}, struct{}](reg, atype.Indexer, provider, name))
}

// DefineRetriever registers the given retrieve function as an action, and returns a
// [Retriever] that runs it.
func DefineRetriever(reg *registry.Registry, provider, name string, ret func(context.Context, *RetrieverRequest) (*RetrieverResponse, error)) *retrieverActionDef {
	return (*retrieverActionDef)(core.DefineAction(reg, provider, name, atype.Retriever, nil, ret))
}

// IsDefinedRetriever reports whether a [Retriever] is defined.
func IsDefinedRetriever(reg *registry.Registry, provider, name string) bool {
	return (*retrieverActionDef)(core.LookupActionFor[*RetrieverRequest, *RetrieverResponse, struct{}](reg, atype.Retriever, provider, name)) != nil
}

// LookupRetriever looks up a [Retriever] registered by [DefineRetriever].
// It returns nil if the model was not defined.
func LookupRetriever(reg *registry.Registry, provider, name string) Retriever {
	return (*retrieverActionDef)(core.LookupActionFor[*RetrieverRequest, *RetrieverResponse, struct{}](reg, atype.Retriever, provider, name))
}
