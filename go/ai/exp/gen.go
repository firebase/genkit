...
const (
	...
	// context, and the abort is best effort from there: the finalize writes how
	// the work actually ended, with the state through the last committed turn.
	// [SnapshotStatusAborted] means the stop ended it, [SnapshotStatusCompleted]
	// that the turn still returned a result, and [SnapshotStatusFailed] that it
	// broke on its own before the stop reached it. Inputs queued behind the turn
	// that was in flight never start. It is not terminal, so a wait keeps
	// waiting, and it is not resumable: the row carries no state yet, and
	// resuming from the parent snapshot would fork away from the work the
	// finalize is about to commit. The worker keeps refreshing
	// [SessionSnapshot.HeartbeatAt] while it drains, so a stale beat means it died
	// between the two writes and a read surfaces the row as
