...
func DefineIndexerAndRetriever(reg *registry.Registry, name string, cfg Config) (ai.Indexer, ai.Retriever, error) {
	ds, err := newDocStore(cfg.Dir, name, cfg.Embedder, cfg.EmbedderOptions)
	if err != nil {
		return nil, nil, err
	}
	return ai.DefineIndexer(reg, provider, name, ds.index),
		ai.DefineRetriever(reg, provider, name, ds.retrieve),
		nil
}
