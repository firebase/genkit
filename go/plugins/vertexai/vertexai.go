...
func Init(ctx context.Context, reg *registry.Registry, cfg *Config) error {
	if cfg == nil {
		cfg = &Config{}
	}
	state.mu.Lock()
...
func defineEmbedder(reg *registry.Registry, name string) ai.Embedder {
	...
	return ai.DefineEmbedder(reg, provider, name, func(ctx context.Context, req *ai.EmbedRequest) (*ai.EmbedResponse, error) {
